...
def test_safe_defaults_require_no_provider_secret() -> None:
    ...
    assert settings.database_workspace_upload_max_bytes == 100_000_000
    assert settings.database_workspace_max_database_bytes == 200_000_000
...
def test_query_budget_validation_fails_closed() -> None:
    ...
    with pytest.raises(ValidationError):
        AppSettings(database_workspace_upload_max_bytes=100_000_001, _env_file=None)

    with pytest.raises(ValidationError):
        AppSettings(database_workspace_max_database_bytes=200_000_001, _env_file=None)
